...
function SearchBarCont() {
  ...
  return (
    <div>
      <div className='mt-10 bg-customOrange p-2 animate-fadeInUp'>
        <Container>
      ...
      {showResults && (
        <div ref={resultsRef} className="relative">
          <div className="container mx-auto p-4 bg-gradient-to-b from-customOrange to-customWhite">
            ...
            <div className="overflow-y-auto max-h-96 mt-4 scrollbar-hide">
                <div className="flex flex-col">
                  {searchPerformed && products.length === 0 && <p className='text-xl font-medium'>Aucun produit trouvé!</p>}
                  {products.map(product => (
                    <div key={product.ID} className="flex items-start border rounded-lg shadow-lg bg-customWhite p-4 mb-2 hover:border-customGreen cursor-pointer hover:border-2 transition duration-300">
                      ...
                      >
                        ...
                        />
                        <div className="flex flex-col">
                          <h2 className="text-lg font-bold text-customDarkGreen mb-2">{product.title}</h2>
                          <p className="text-sm text-gray-600"><strong>Marque : </strong>{product.brand}</p>
                          <p className="text-xs mt-2 text-gray-600"><strong>Détails : </strong> {product.details}</p>
                          <p className="text-sm mt-2 text-gray-600"><strong className='font-semibold'>Prix : </strong> {product.price}</p>
                        </div>
                      </Link>
                    </div>
                  ))}
