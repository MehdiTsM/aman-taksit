    ...
    <header className="bg-customDarkGreen p-4 md:p-7 h-auto md:h-20 animate-fadeInUp font-medium">
      <div className="flex flex-col md:flex-row justify-around items-center space-y-4 md:space-y-0">
        ...
        <nav className="flex flex-col md:flex-row md:space-x-8 items-center space-y-4 md:space-y-0">
          ...
          <NavLink 
            to="/simulation" 
            className={({ isActive }) => `text-lg md:text-xl ${isActive ? 'text-customOrange' : 'text-customWhite'} relative border-b-2 border-transparent hover:text-customOrange transition-colors duration-300 hover:border-customOrange pb-2 md:pb-6`}          >netlify login
            Simulation
          </NavLink>
        </nav>
      </div>
