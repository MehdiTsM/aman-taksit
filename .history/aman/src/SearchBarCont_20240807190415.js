...
function SearchBarCont() {
  ...
  return (
    <div>
      ...
      {showResults && (
        <div ref={resultsRef} className="container mx-auto p-4 bg-gradient-to-b from-customOrange to-customWhite">
          <h2 className="text-2xl font-bold text-customOrange mb-4">Search Results</h2>
          <div className="overflow-auto max-h-80">
            {products.length === 0 && <p>No products found</p>}
            {products.map(product => (
              <div key={product.ID} className="flex border rounded-lg shadow-lg bg-customWhite p-4 mb-4 hover:border-customOrange hover:border-2 transition-all duration-300">
                <img 
