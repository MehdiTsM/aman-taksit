...
function ProductDetail() {
  ...
  console.log('Product ID:', id); // Log the ID to verify it

  const [product, setProduct] = useState(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch(`http://localhost:8081/product/${id}`) // Ensure the endpoint matches your backend route
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="border border-gray-300 rounded-lg shadow-lg bg-customWhite p-4">
        <img
