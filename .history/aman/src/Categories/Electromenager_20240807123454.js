        ...
        {products.map(product => (
          <div key={product.ID} className="border rounded-lg shadow-lg bg-customWhite p-4 hover:shadow-2xl transition-shadow duration-300 ">
            ...
            <div className="flex flex-col">
              ...
              <p className="text-lg mt-2 text-customDarkGreen"><strong className='font-semibold'>Prix :</strong> {product.price}</p>
              <p className='text-customDarkGreen text-2xl'>Voir plus</p>
            </div>
          </div>
        ))}
      </div>
