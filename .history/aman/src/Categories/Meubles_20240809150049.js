...
function Meubles() {
  ...
  useEffect(() => {
    fetch('http://localhost:8081/products/meubles')
  ...
  return (
    <div className="container mx-auto p-4 mb-20">
      <h1 className="text-3xl md:text-4xl font-semibold text-customDarkGreen mb-8 mt-8">Meubles :</h1>
        ...
        {products.map(product => (
          <Link to={`/product/${product.ID}`} key={product.ID} className="border border-gray-300 rounded-lg shadow-lg bg-customWhite p-4 hover:border-customOrange hover:shadow-2xl transition-all duration-300">
            ...
            <div className="flex flex-col">
              <h2 className="text-xl mt-2 font-bold text-customOrange mb-2 truncate">{product.title}</h2>
              <p className="text-lg"><strong>{product.brand}</strong></p>
              <p className="text-sm mt-2 text-gray-600 truncate"><strong>Detail :</strong> {product.details}</p>
