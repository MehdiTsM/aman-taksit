  ...
  return (
    <div>
      ...
      <div className="container mx-auto p-4">
        <h2 className="text-2xl font-bold text-customOrange mb-4">Search Results</h2>
        <div className="max-h-96 overflow-y-auto border rounded-lg bg-customWhite p-4 shadow-md">
          {!searchPerformed && <p className="text-gray-500">Search for products to see results.</p>}
          {searchPerformed && products.length === 0 && <p className="text-gray-500">No products found</p>}
          {products.map(product => (
            <div key={product.ID} className="flex items-center border-b last:border-b-0 mb-4 pb-4">
              <img 
              ...
              <div className="flex-grow">
                <h2 className="text-lg font-bold text-customOrange mb-1">{product.title}</h2>
                <p className="text-sm text-gray-700"><strong>Brand:</strong> {product.brand}</p>
                <p className="text-xs mt-1 text-gray-600"><strong>Details:</strong> {product.details}</p>
                <p className="text-sm mt-1 text-gray-700"><strong className='font-semibold'>Price:</strong> {product.price}</p>
              </div>
            </div>
          ))}
        </div>
